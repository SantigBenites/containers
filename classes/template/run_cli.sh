...
if ! apptainer instance list | grep -cq "DEFAULT"; then
    apptainer instance start /opt/sifs/DEFAULT.sif DEFAULT
fi
...
if [[ "$RUN_TARGET" == "code" ]]; then
    ...
    apptainer shell instance://DEFAULT
elif [[ "$RUN_TARGET" == "stop" ]]; then
    apptainer instance stop DEFAULT
fi
